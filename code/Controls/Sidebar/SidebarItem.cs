// Copyright (c) Microsoft Corporation and Contributors. Licensed under the MIT License.
// Copyright (c) Files Community. Licensed under the MIT License.
// 照抄 Files 4.2.3 (src\Files.App.Controls\Sidebar\SidebarItem.cs)
...
public sealed partial class SidebarItem : Control
{
	...
	private void HookupOwners()
	{
		...
		// Static SidebarItems (e.g. a settings button inside SidebarView.Footer) sit outside MenuItemsHost, so SidebarView.OnSelectedItemChanged's broadcast can't reach them. The per-row callback fills that gap.
	...
	private void SetFlyoutOpen(bool isOpen = true)
	{
		...
		// ArgumentException when GetAttachedFlyout/ShowAttachedFlyout runs before the template is applied (e.g. DisplayMode toggled at startup, before all containers have realized).
